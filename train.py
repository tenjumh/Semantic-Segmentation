...
parser.add_argument('--frontend', type=str, default="InceptionV4", help='The frontend you are using. See frontend_builder.py for supported models')
...
opt = tf.train.RMSPropOptimizer(learning_rate=0.0002, decay=0.995).minimize(loss, var_list=[var for var in tf.trainable_variables()])
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    if epoch % args.validation_step == 0:
        ...
        for ind in val_indices:
            ...
            input_image = np.expand_dims(np.float32(utils.load_image(val_input_names[ind])[33:33 + args.crop_height, 60:60 + args.crop_width]),axis=0)/255.0
            gt = utils.load_image(val_output_names[ind])[33:33 + args.crop_height, 60:60 + args.crop_width]
